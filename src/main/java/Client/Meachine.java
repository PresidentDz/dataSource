...
public class Meachine {
	...
	void getchess_X_Y(){//??????????λ????????λ??????????
		setInitia();
		data=ClientService.getClientService().getGameData();
		for (int i = 0; i < data.length; i++) {//??????????е?????λ?????
			for (int j = 0; j < data[0].length; j++) {//??????????е????????
				switch (data[i][j]) {
				case 111://?ж?  ??
					x_zu[0]=i;
					y_zu[0]=j;
	...
	boolean isAttacked(int i,int j,int type){//????????ж????λ?????????
